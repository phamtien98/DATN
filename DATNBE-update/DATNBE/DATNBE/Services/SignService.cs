...
namespace DATNBE.Services
{
    public class SignService : IServices
    {
        ...
        public void SaveCertFile(IFormFile formFile)
        {
            ...
            {
                //Get url To Save
                string SavePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/CertFile", formFile.FileName + DateTime.Now.ToString("yyyyMMddss"));
        ...
        public void SaveHandWritting(IFormFile formFile)
        {
            if (formFile != null)
            {
                ...
                //Get url To Save
                string SavePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/HandWritting", formFile.FileName + DateTime.Now.ToString("yyyyMMddss"));
        ...
        public string Encrypt(IFormFile pdfFile, IFormFile CertOwner, string passOwner, IFormFile CertUser, string passUser, int number)
        {
            ...
            using (PdfDocument pdf = new PdfDocument(path1))
            {
                ...
                PdfPublicKeyEncryptionHandler handler = createEncryptionHandler(CertOwner,passOwner, CertUser,passUser);
                switch (number)
                {
                    case 1:
        ...
        private PdfPublicKeyEncryptionHandler createEncryptionHandler(IFormFile CertOwner, string passOwner, IFormFile CertUser, string passUser)
        {
            ...
            permisssions2.AllowEverything = false;
        ...
        public string Decrypt(IFormFile pdfFile, IFormFile certFile, string password)
        {
            try
            {
                ...
                var encryptedFile = Path.Combine("wwwroot/PDFFile", certFile.FileName);
